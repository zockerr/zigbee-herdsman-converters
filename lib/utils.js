...
function applyColorMode(result, meta) {
    if (meta.options && meta.options.color_mode === 'exclude') {
        if (result.state.hasOwnProperty('color_temp')) {
            result.state.color = undefined;
        } else if (result.state.hasOwnProperty('color')) {
            result.state.color_temp = undefined;
        }
    } else {
        // default = 'convert'
        if (result.state.hasOwnProperty('color_temp')) {
            result.state.color = miredsToXY(result.state.color_temp);
        } else if (result.state.hasOwnProperty('color') && result.state.color.hasOwnProperty('x') &&
            result.state.color.hasOwnProperty('y')) {
            result.state.color_temp = xyToMireds(result.state.color.x, result.state.color.y);
        }
    }
}
